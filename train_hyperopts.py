...
search_space = {
    "optimizer": hp.choice('optmz',["sgd", "rms", "adam", "amsgrad"]),
    "lr": hp.uniform('lr',1e-5,1e-3),
    "decay": hp.choice('decay', [0.0001, 0.0005, 0.0009]),
    "momentum": hp.uniform("momentum", 0.01, 1),
    "lr_steps": hp.choice('lr_steps', [(28000, 60000, 70000, 75000), (20000, 40000, 60000)])
}
...
parser.set_defaults(keep_latest=False, log=True, log_gpu=False, interrupt=True, autoscale=False)
...
if args.autoscale and args.batch_size != 8:
    ...
    cfg.lr *= factor
    cfg.max_iter //= factor
    lr_steps = [x // factor for x in lr_steps]
...
def train(params):
    ...
    try:
        ...
        with mlflow.start_run():
            ...
            ### HyperOpt get parameter
            lr = params['lr']
            decay = params['decay']
            momentum = params['momentum']
            lr_steps = params['lr_steps']

            ### Mlflow log Optimizer
            ...
            if params['optimizer'] == 'sgd':
                ...
                optimizer = optim.Adam(net.parameters(), lr=lr, weight_decay=decay, amsgrad=True)
            ...
            try:
                for epoch in range(num_epochs):
                    ...
                    for datum in data_loader:
                        ...
                        if cfg.lr_warmup_until > 0 and iteration <= cfg.lr_warmup_until:
                            set_lr(optimizer, (lr - cfg.lr_warmup_init) * (iteration / cfg.lr_warmup_until) + cfg.lr_warmup_init)

                        # Adjust the learning rate at the given iterations, but also if we resume from past that iteration
                        while step_index < len(lr_steps) and iteration >= lr_steps[step_index]:
                            step_index += 1
                            set_lr(optimizer, lr * (args.gamma ** step_index))
                ...
                val_map = compute_validation_map(epoch, iteration, yolact_net, val_dataset, log if args.log else None)
            ...
            yolact_net.save_weights(save_path(epoch, repr(iteration) + '_' + mlflow.active_run().info.run_id))
            return { 'loss': 100-val_map, 'status': STATUS_OK}
    except RuntimeError:
...
def compute_validation_map(epoch, iteration, yolact_net, dataset, log:Log=None):
    with torch.no_grad():
        ...
        mlflow.log_metric('mAP Box', val_info['box']['all'])
        mlflow.log_metric('mAP Mask', val_info['mask']['all'])
        end = time.time()
